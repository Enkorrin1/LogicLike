...
- upgraded `tool/content_audit.dart` into a release gate that exits with a
  non-zero code when content issues are found,
- added structured release-gate checks and issue details to `CONTENT_AUDIT.json`,
- audit now checks RU/EN localization presence, mojibake, correct answers,
  missing visual data, local SVG art, content assets, detail-count metadata,
  nearby duplicate puzzle signatures, and difficulty progression,
- fixed early generated content variants so close starter lessons no longer
  repeat the same exact shape or memory-pair puzzle,
- added automated test coverage that runs the content audit as part of release
  QA and verifies the gate passes.
